...
PlyProperty vert_props[] = { /* list of property information for a vertex */
  ...
  {"status", Int32, Int32, offsetof(Vertex_io, status), 0, 0, 0, 0}
};
  ...
  for (i = 0; i < in_ply->num_elem_types; i++) {
    ...
    if (equal_strings ("vertex", elem_name)) {
      ...
      for (j = 0; j < nverts; j++) {
		...
		//vlist[j] = new Vertex (vert.y, vert.z, vert.x); //For the abc flow slice
		vlist[j]->vx = vert.vx;
		vlist[j]->vy = vert.vy;
		vlist[j]->vz = vert.vz;
		vlist[j]->vec.set(vert.vx, vert.vy);
       vlist[j]->other_props = vert.other_props;
	   vlist[j]->inside = true;
	   vlist[j]->boundary = false;
...
void Polyhedron::normalized_domain() // normalize the domain to between [0,1]x[0,1]
{
	unsigned int i;

	for (i=0; i<nverts; i++)
	{
		vlist[i]->x = (vlist[i]->x - center.entry[0])/(sqrt(2.)*radius) + 0.5;
		vlist[i]->y = (vlist[i]->y - center.entry[1])/(sqrt(2.)*radius) + 0.5;
	}
}
...
void Polyhedron::calc_edge_length()
{
	...
	//mark boundary edges
		if (elist[i]->ntris == 1)
		{
			elist[i]->verts[0]->boundary = true;
			elist[i]->verts[1]->boundary = true;
		}
	}
